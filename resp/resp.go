...
func testString(s string) {
	...
	writeIOBuffer := bufio.NewWriter(&bytebuffer)
	request := RespWriter{bw: writeIOBuffer}

	request.FlushString(s)
	fmt.Println("intput==============================", bytebuffer.Bytes(), bytebuffer.String())

	readerIOBuffer := bufio.NewReader(&bytebuffer)
	response := RespReader{br: readerIOBuffer}

	data, err := response.Parse()

	if err != nil {
		fmt.Println("err:", err)
	}
	fmt.Println("output===================================", reflect.TypeOf(data))

	fmt.Println("output===================================", data.(string), len(data.(string)))
}

func testInt(n int64) {
	var bytebuffer bytes.Buffer

	writeIOBuffer := bufio.NewWriter(&bytebuffer)
	request := RespWriter{bw: writeIOBuffer}

	request.FlushInteger(n)
	fmt.Println("intput==============================", bytebuffer.Bytes(), bytebuffer.String())
...
func testError(e error) {
	...
	fmt.Println("output===================================", data.(error))
}

func testArray(ay []interface{}) {
	var bytebuffer bytes.Buffer

	writeIOBuffer := bufio.NewWriter(&bytebuffer)
	request := RespWriter{bw: writeIOBuffer}

	request.FlushArray(ay)
	fmt.Println("intput==============================", bytebuffer.Bytes(), bytebuffer.String())
...
func testbulk(cmd string, args ...interface{}) {
	var bytebuffer bytes.Buffer

	writeIOBuffer := bufio.NewWriter(&bytebuffer)
	request := RespWriter{bw: writeIOBuffer}
	request.WriteCommand(cmd, args)
	fmt.Println("intput==============================", bytebuffer.Bytes(), bytebuffer.String())
	...
	for _, v := range ans {
		fmt.Println("output===================================", string(v.([]uint8)))
	}
...
func main() {
	testString("Ok")
	testInt(10000)
	testError(errors.New("a new error"))
	testArray([]interface{}{"haha", "hehe", "hengheng"})
	testbulk("login", 100, "name", "password")
}
